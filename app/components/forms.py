...
class ComponentForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "crafting_time": forms.NumberInput(
                attrs={
                    "class": "form-control",
    ...
    def clean_materials(self):
        ...
        if not isinstance(materials, dict):
            raise ValidationError(
                "Materials must be a dictionary of ore_id: quantity pairs."
            )
        ...
        for ore_id_str, quantity in materials.items():
            ...
            try:
                ...
                raise ValidationError(
                    f"Invalid quantity for ore {ore_id_str}: {quantity}"
                )

            if quantity_float <= 0:
                raise ValidationError(
                    f"Quantity for ore {ore_id_str} must be positive (got {quantity_float})."
                )
