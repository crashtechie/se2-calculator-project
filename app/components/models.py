...
class Component(models.Model):
    ...
    crafting_time = models.FloatField(
        default=0.0, help_text="Time required to craft in seconds"
    )

    mass = models.FloatField(
        default=0.0, help_text="Total mass of the component in kilograms"
    )

    created_at = models.DateTimeField(
        auto_now_add=True, help_text="Timestamp when the component was created"
    )

    updated_at = models.DateTimeField(
        auto_now=True, help_text="Timestamp when the component was last updated"
    )
    ...
    def validate_materials(self):
        ...
        for ore_id_str, quantity in self.materials.items():
            try:
                ...
                try:
                    Ore.objects.get(ore_id=ore_id_str)
                except Ore.DoesNotExist:
                    errors.append(f"Ore with ID {ore_id_str} does not exist")
    ...
    def clean(self):
        ...
        if not is_valid:
            raise ValidationError(f"Materials validation failed: {', '.join(errors)}")
