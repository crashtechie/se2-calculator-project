...
class ComponentAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Basic Information", {"fields": ("name", "description", "fabricator_type")}),
        (
    ...
    def materials_preview(self, obj):
        ...
        return mark_safe(
            '<span title="{}">{} material{}</span>'.format(
                json.dumps(obj.materials), material_count, plural
            )
        )
    ...
    def material_ores(self, obj):
        ...
        ore_items = [
            "<strong>{}</strong>: {} units".format(
                ore.name, obj.materials[str(ore.ore_id)]
            )
            for ore in ores
        ]
    ...
    def validation_status(self, obj):
        ...
        if is_valid:
            ...
            return mark_safe(
                '<span style="color: red; font-weight: bold;">✗ Invalid</span><br>{}'.format(
                    error_list
                )
            )
