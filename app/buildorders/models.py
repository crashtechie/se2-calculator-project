...
class BuildOrder(models.Model):
    ...
    name = models.CharField(max_length=200, help_text="Name of the build order")
    ...
    def validate_blocks(self):
        ...
        for block_id, quantity in self.blocks.items():
            try:
                ...
                if quantity <= 0:
                    errors.append(
                        f"Invalid quantity for block {block.name}: {quantity}"
                    )
    ...
    def _get_components_with_details(self):
        ...
        for comp_id, qty in components.items():
            try:
                ...
                result.append(
                    {"component": comp, "quantity": qty, "total_mass": comp.mass * qty}
                )
            except Component.DoesNotExist:
                continue
        return result
