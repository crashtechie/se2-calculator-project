...
class Block(models.Model):
    ...
    description = models.TextField(
        blank=True, help_text="Detailed description of the block and its uses"
    )
    ...
    consumer_rate = models.FloatField(
        default=0.0, help_text="Consumption rate per second"
    )
    ...
    producer_rate = models.FloatField(
        default=0.0, help_text="Production rate per second"
    )

    storage_capacity = models.FloatField(
        default=0.0, help_text="Storage capacity in liters or units"
    )

    created_at = models.DateTimeField(
        auto_now_add=True, help_text="Timestamp when the block was created"
    )

    updated_at = models.DateTimeField(
        auto_now=True, help_text="Timestamp when the block was last updated"
    )
        ...
        for component_id, quantity in self.components.items():
            try:
                ...
                try:
                    ...
                    errors.append(f"Component with ID {component_id} does not exist")
    ...
    def iter_component_requirements(self):
        ...
        for comp_id, quantity in (self.components or {}).items():
            comp = (
                Component.objects.filter(component_id=comp_id).first()
                if comp_id
                else None
            )
