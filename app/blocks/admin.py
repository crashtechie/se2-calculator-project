...
class BlockAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Basic Information", {"fields": ("name", "description", "mass")}),
        (
            "Block Properties",
        ...
        (
            ...
            {
                "fields": ("components", "components_formatted", "component_objects"),
        ...
        ("Validation", {"fields": ("validation_status",), "classes": ("collapse",)}),
        (
            "System Information",
            {
    ...
    def components_preview(self, obj):
        ...
        return mark_safe(
            '<span title="{}">{} component{}</span>'.format(
                json.dumps(obj.components), component_count, plural
            )
        )
    ...
    def component_objects(self, obj):
        ...
        quantity_map = {
            item["component_id"]: item["quantity"] for item in obj.components
        }

        component_list = "<br>".join(
            [
                "<strong>{}</strong>: {} units".format(
                    comp.name, quantity_map.get(str(comp.component_id), 0)
                )
    ...
    def consumer_info(self, obj):
        ...
        return mark_safe(
            "<strong>{}</strong><br>{} /s".format(obj.consumer_type, obj.consumer_rate)
        )
    ...
    def producer_info(self, obj):
        ...
        if not obj.producer_type:
            return mark_safe("<em>None</em>")
        return mark_safe(
            "<strong>{}</strong><br>{} /s".format(obj.producer_type, obj.producer_rate)
        )
