...
class BlockListView(ListView):
    ...
    def get_queryset(self):
        ...
        logger.debug(
            f"BlockListView query: search='{search_query}', sort={sort_by}, order={order}, count={queryset.count()}"
        )
        return queryset
...
class BlockDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        logger.info(
            f"BlockDetailView: {block.name} with {len(block.components or {})} components"
        )
        return context
    ...
    def _calculate_resource_chain(self, block):
                ...
                if component.materials:
                    for ore_id, ore_quantity in component.materials.items():
                        try:
                            ...
                            logger.warning(
                                f"Ore {ore_id} not found for component {component.name}"
                            )
        ...
        total_ore_mass = sum(
            ore["quantity"] * ore["mass"] for ore in ore_totals.values()
        )
    ...
    def _calculate_stats(self, block, resource_chain):
        ...
        return {
            ...
            "total_component_quantity": sum(block.components.values())
            if block.components
            else 0,
...
class BlockCreateView(CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["available_components"] = context[
            "components_list"
        ]  # Alias for templates
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, f'Block "{self.object.name}" created successfully!'
        )
    ...
    def form_invalid(self, form):
        ...
        messages.error(
            self.request, "Error creating block. Please check the form for errors."
        )
...
class BlockUpdateView(UpdateView):
    ...
    def form_valid(self, form):
        ...
        response = super().form_valid(form)
        messages.success(
            self.request, f'Block "{self.object.name}" updated successfully!'
        )
    ...
    def form_invalid(self, form):
        ...
        messages.error(
            self.request, "Error updating block. Please check the form for errors."
        )
...
class BlockDeleteView(DeleteView):
    ...
    def form_valid(self, form):
        ...
        response = super().form_valid(form)
        messages.success(self.request, f'Block "{block_name}" deleted successfully!')
