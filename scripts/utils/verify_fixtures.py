...
def verify_fixtures():
    ...
    try:
        ...
        print("  ✅ Loaded components fixture")
    except Exception as e:
    ...
    if invalid_material_refs:
        errors.append(
            f"❌ Invalid ore references in components: {', '.join(invalid_material_refs[:3])}"
        )
    else:
    ...
    if invalid_component_refs:
        errors.append(
            f"❌ Invalid component references in blocks: {', '.join(invalid_component_refs[:3])}"
        )
    else:
        ...
        for ore_id in comp["fields"].get("materials", {}).keys():
            if placeholder_pattern.search(ore_id):
                placeholders_found.append(
                    f"component material: {comp['fields']['name']}"
                )
    ...
    if placeholders_found:
        errors.append(
            f"❌ Placeholder UUIDs still present: {', '.join(placeholders_found[:3])}"
        )
    else:
